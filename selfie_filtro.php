...
<body>
  ...
  <canvas id="overlayCanvas" style="position: absolute; top: 0; left: 0; width: 100%; max-width: 480px; aspect-ratio: 4/3; pointer-events: none;"></canvas>
...
function onResults(results) {
  ...
  // Ejemplo de pintura facial: pintar mejillas
  overlayCtx.strokeStyle = "rgba(255, 0, 0, 0.7)";
  overlayCtx.lineWidth = 5;

  overlayCtx.beginPath();
  // Unimos algunos puntos de la cara (esto puedes personalizarlo)
  faceLandmarks.forEach((landmark, index) => {
    if (index === 0) {
      overlayCtx.moveTo(landmark.x * overlayCanvas.width, landmark.y * overlayCanvas.height);
    } else {
      overlayCtx.lineTo(landmark.x * overlayCanvas.width, landmark.y * overlayCanvas.height);
    }
  });
